...
const insertDocuments = async (documents = [{ a: 1 }, { a: 2 }, { a: 3 }]) => {
  // check params
  if (!db || !documents)
...
const findDocuments = async (query = { a: 3 }) => {
  // check params
  if (!db) throw Error('findDocuments::missing required params');
...
const removeDocuments = async (docFilter = {}) => {
  // check params
  if (!db) throw Error('removeDocuments::missing required params');
...
const connectToDatabase = async () => {
  try {
    if (!DATABASE_URL || !DATABASE_NAME) {
      ...
      throw new Error('Database params are missing');
    }
